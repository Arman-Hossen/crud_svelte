...
<script>
    ...
    onMount(async () => {
      const unsubscribe = page.subscribe(async ($page) => {
        const { id } = $page.params; // Destructure id from $page.params
    ...
    async function fetchCategory(id) {
      try {
        const res = await axios.get(`http://10.0.0.85/api/categories/${id}`);
        category = res.data;
    ...
    async function updateCategory() {
      try {
        await axios.put(`http://10.0.0.85/api/categories/${category.id}`, {
